...
python code/temporal_shift_infer.py \
    ...
    --save_file_name ${task1_identity}/${task2_identity}/lr_${time_lr}_seed_${time_seed}_epochs_${epochs}_temperature_${temperature}_${strategy}_re_${re_weight}_ne_${ne_weight}_com_${com_weight}_node \
    --strategy $strategy \
    --time_seed $time_seed \
    --k $k \
    --mode test \
...
python code/test_task2.py \
    ...
    --load_state_path outputs/${owner_id}_checkpoints/task2/$data/${task1_identity}.pt \
    --load_node_embedding_path outputs/${owner_id}_node_embedding/task2/$data/train/${task1_identity}_node.pt \
    --load_time_embedding_path outputs/${owner_id}_time_embedding/task2/$data/test/$month/${task1_identity}/${task2_identity}/lr_${time_lr}_seed_${time_seed}_epochs_${epochs}_temperature_${temperature}_${strategy}_re_${re_weight}_ne_${ne_weight}_com_${com_weight}_node.pt \
    --fix_model yes \
    --con_weight 0.0 \
    --rank_weight $rank_weight \
    --lp_weight $lp_weight \

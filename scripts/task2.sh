...
python code/train_task1.py \
    ...
    --train_path data/$data/$task/0_${start_date}/triplet_percentage.tsv \
    --eval_path data/$data/$task/0_${start_date}/triplet_percentage.tsv \
    --test_path data/$data/$task/0_${start_date}/triplet_percentage.tsv \
    --task $task \
    --k $task1_k \
...
task1_identity=epoch_${task1_num_epochs}_k_${task1_k}_lr_${task1_lr}_initalembed_yes_seed_$task1_seed/rglossfn_${task1_rg_loss_fn}_activate_softplus_rgweight_${task1_rg_weight}_lpweight_${task1_lp_weight}_rankweight_${task1_rank_weight}_conweight_${task1_con_weight}_gaussian_yes_crossattn_${task1_bias}_bias_${task1_bias}_0_${start_date}
